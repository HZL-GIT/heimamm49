  ...
  <el-container class="layout">
    ...
    <el-container>
      ...
      <el-aside width="auto" class="aside">
        ...
        >
        <!-- 
          $router.options.routes[1].children通过打印thi.router查看当前路由的所有信息获取 
        -->
          <el-menu-item
...
export default {
  ...
  created() {
    ...
    getUserInfo().then(res => {
      ...
      /* 
      用户权限判断小结
      1.获取到登录者的信息后，判断登录者是的账号状态(status)是否为启用(1)状态，如果是禁用(0)状态就提示用户，再清除他的token并退回到登录界面(this.$router.push('/'))
      ...
      4.由于在上一步的权限判断是在created中定义的，他只会执行一次。所以在用户登录之后，还是可以访问不属于他权限的内容，所以，需要在路由前守卫中再次进行权限判断，判断前，必须先给vuex中的role一个初始值，因为如果没有初始值，用户根本进不来，因为路由前守卫比钩子函数的执行时机要更早。所以先在获取用户信息的时候，将用户角色信息保存到vuex中新定义的role上，且role需要默认先给最大权限，让用户能够进来再说，在获取到真实角色时再处理。
      5.对于用户无权限访问的组件切换按钮，需要进行v-if或v-show进行显示与隐藏的处理，注意如果该按钮使用了v-for循环进行渲染的话，只能使用v-show进行处理，使用v-if处理会报错
      */
